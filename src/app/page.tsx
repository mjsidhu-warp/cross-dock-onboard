...
export default function PortalPage() {
  ...
  const setupSteps = [
    { done: rateAccepted, label: "Rate terms accepted" },
    { done: mobileOk, label: "Mobile app connected" },
    { done: desktopOk, label: "Desktop app connected", optional: true },
    { done: scanInOk, label: "Test scan-in completed" },
    { done: scanOutOk, label: "Test scan-out completed" },
    { done: labelOk, label: "Label print test passed" },
    { done: goLiveRequested, label: "Go-live requested" },
  ];
  const setupDone = setupSteps.filter((s) => s.done).length;
  const requiredDone = setupSteps.filter((s) => !s.optional && s.done).length;
  const requiredTotal = setupSteps.filter((s) => !s.optional).length;
  const canGoLive = requiredDone === requiredTotal - 1; // all except go-live itself
    ...
    <div className="min-h-screen bg-warp-bg">
                    ...
                    {isOpen && (
                      <div className="border-t border-warp-border px-6 pb-6 pt-4">
                        ...
                        {sec.key === "tech" && (
                          <div className="space-y-4">
                            {[
                              { key: "hasDevices" as const, q: "Do you have a phone or tablet for scanning?", hint: "iPhone, Android, or tablet with a camera" },
                              { key: "hasLabelPrinter" as const, q: "Do you have a label printer?", hint: "Thermal or inkjet for pallet labels" },
                              { key: "hasFloorInternet" as const, q: "Wi-Fi or cell signal on the dock floor?", hint: "Needed for real-time scan sync" },
                            ].map((item) => (
      ...
      <section id="setup" className="border-t border-warp-border py-20">
        <div className="mx-auto max-w-3xl px-6">
          ...
          <div className="mt-6 space-y-2">
            {/* Rate terms */}
            <ChecklistRow done={rateAccepted} label="Accept rate terms" desc="Review the standard Warp per-pallet rate card." icon={FileText} optional={false}>
              {!rateAccepted && <GreenBtn onClick={() => setShowRateModal(true)}>View rate card</GreenBtn>}
            </ChecklistRow>

            {/* Mobile */}
            <ChecklistRow done={mobileOk} label="Access mobile scanner app" desc="Open wh.wearewarp.com on your phone and sign in." icon={Smartphone} optional={false}>
              {!mobileOk && (
                <div className="flex gap-2">
                  <a href="https://wh.wearewarp.com" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-lg border border-warp-border px-3 py-1.5 text-xs text-warp-secondary hover:border-warp-border-light">Open app <ExternalLink className="h-3 w-3" /></a>
            ...
            {/* Desktop */}
            <ChecklistRow done={desktopOk} label="Access desktop app" desc="Open warehouse.wearewarp.com on your computer." icon={Monitor} optional>
              {!desktopOk && (
                <div className="flex gap-2">
                  <a href="https://warehouse.wearewarp.com" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-lg border border-warp-border px-3 py-1.5 text-xs text-warp-secondary hover:border-warp-border-light">Open app <ExternalLink className="h-3 w-3" /></a>
            ...
            {/* Test scans */}
            <ChecklistRow done={scanInOk} label="Complete a test scan-in" desc="Scan a test pallet into your facility." icon={ScanBarcode} optional={false}>
              {!scanInOk && <GreenBtn onClick={() => setScanInOk(true)}>Mark done</GreenBtn>}
            </ChecklistRow>
            <ChecklistRow done={scanOutOk} label="Complete a test scan-out" desc="Scan the same test pallet out." icon={ScanBarcode} optional={false}>
              {!scanOutOk && <GreenBtn onClick={() => setScanOutOk(true)}>Mark done</GreenBtn>}
            </ChecklistRow>

            {/* Label */}
            <ChecklistRow done={labelOk} label="Print a test label" desc="Print a pallet label from the app." icon={Printer} optional={false}>
              {!labelOk && <GreenBtn onClick={() => setLabelOk(true)}>Mark done</GreenBtn>}
            </ChecklistRow>

            {/* Go live */}
            <ChecklistRow done={goLiveRequested} label="Request go-live" desc="Submit for Warp review. Once approved, you receive freight." icon={Rocket} optional={false}>
              {!goLiveRequested && (
                <button
      ...
      <section id="operations" className="border-t border-warp-border py-20">
        <div className="mx-auto max-w-6xl px-6">
          ...
          <p className="mt-2 text-warp-secondary">
            Today&apos;s routes, scan compliance, and dock activity.
          </p>
          ...
          {MOCK_SCANS.missedScans.length > 0 && (
            <div className="mt-6 rounded-xl border border-amber-500/30 bg-warp-amber-dim p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-warp-secondary">Missed scans = missed earnings. Scan every pallet to get paid for every pallet.</p>
                </div>
              </div>
            </div>
          )}
      ...
      {/* ═══ FOOTER ═══ */}
      <footer className="border-t border-warp-border py-8">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6">
          ...
          <a href="https://www.wearewarp.com" target="_blank" rel="noopener noreferrer" className="text-xs text-warp-muted hover:text-warp-secondary">
            wearewarp.com
          </a>
        </div>
      </footer>
